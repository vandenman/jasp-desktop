...
extendJaspTableAndCreateJaspResults <- function()
{
	return(function()
	{
		...
		env$jaspResults <- create_jaspResults("Analysis Test")

		setRefClass(
			"jaspTableExtended", 
			contains=c("Rcpp_jaspTable"), 
			methods=list(
				addColumnInfo 	= function(name=NULL, title=NULL, type=NULL, format=NULL, combine=NULL) { addColumnInfoHelper(name, title, type, format, combine)}, 
		...
		return("jaspResults has been created and can now be used to test/develop your analysis, try something like: 'jaspResults$print()' or 'jaspResults[[\"aTable\"]] <- getJaspTable()'.")
	})
}
...
getJaspPlot <- function(plot=NULL, title="", width=320, height=320, aspectRatio=0, error=NULL, errorMessage="", dependencies=NULL)
{
  ...
  jaspPlotObj  <- create_jaspPlot(title) # If we use R's constructor it will garbage collect our objects prematurely.. #new(jaspResultsModule$jaspPlot, title)
  ...
  if(!is.null(plot))
	{
		...
		if(!is.null(writtenImage))
		{
			...
			print(jaspPlotObj$filePathPng)
      		jaspPlotObj$setPlotObjSerialized(serialize(object=plot, connection=NULL, ascii=TRUE))
		}
	}
...
{
  checkForJaspResultsInit()
  return(create_jaspContainer(title)) # If we use R's constructor it will garbage collect our objects prematurely.. #new(jaspResultsModule$jaspContainer, title))
}

getJaspTable <- function(title="", data=NULL, colNames=NULL, colTitles=NULL, colFormats=NULL, rowNames=NULL, rowTitles=NULL, dependencies=NULL)
{
  checkForJaspResultsInit()

  jaspObj <- create_jaspTable(title) # If we use R's constructor it will garbage collect our objects prematurely.. #new(jaspResultsModule$jaspTable, title)
...
getJaspHtml <- function(text="", elementType="p")
{
  ...
  htmlObj <- create_jaspHtml(text) # If we use R's constructor it will garbage collect our objects prematurely.. #
