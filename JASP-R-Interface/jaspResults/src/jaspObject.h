...
enum class jaspObjectType { unknown, container, table, plot, json, list, results, html };
...
public:
	...
	virtual void		childrenUpdatedCallbackHandler()	{} ///Can be caugt by jaspResults to send changes and stuff like that.
			...
			void		dependOnTheseOptions(Rcpp::CharacterVector listOptions);
...
public:
	...
	void		dependOnTheseOptions(Rcpp::CharacterVector listOptions)									{ myJaspObject->dependOnTheseOptions(listOptions); }
...
#define JASP_OBJECT_CREATOR_FUNCTIONNAME_STR(JASP_TYPE) "create_" #JASP_TYPE
...
RCPP_EXPOSED_CLASS(jaspObject_Interface)
